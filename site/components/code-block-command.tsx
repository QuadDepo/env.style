 ...
 * and bundled SVG icons.
 * This project's surface was trimmed to only the props its caller uses.
 *
 * Props:
 * - pnpm/yarn/npm/bun?: command strings
 *
...
function DefaultIcon({ svg }: { svg: string }) {
	return (
		<span
			className="inline-flex size-3.5 shrink-0 [&>svg]:size-full"
			aria-hidden="true"
...
export function CodeBlockCommand({
	...
	// ponytail: post-hydration localStorage sync — accepted flash beats hydration-mismatch complexity
	React.useEffect(() => {
		try {
			const stored = localStorage.getItem(STORAGE_KEY) as PackageManager | null;
			if (stored && commands[stored]) setActive(stored);
		} catch {
			// localStorage unavailable (private mode, disabled storage, etc.) — keep the default tab
		}
	}, [commands]);
	...
	function handleSelect(manager: PackageManager) {
		...
		if (typeof window !== "undefined") {
			try {
				...
				// localStorage unavailable (private mode, disabled storage, etc.) — selection just won't persist
			}
		}
	}
	...
	function renderIcon(manager: PackageManager) {
		return <DefaultIcon svg={defaultIcons[manager]} />;
	}
