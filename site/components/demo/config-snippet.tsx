...
import { CONFIG_FILES, useDemo } from "./provider";
...
export function ConfigSnippet({ option }: { option: SnippetBlock }) {
	...
	return (
		<div className="flex flex-col gap-2">
			{/* mobile: the mini tab-strip preview sits flush on top, so square the top corners to read as one card */}
			<CodeBlock
				label={CONFIG_FILES[file]}
				actions={<FrameworkSelect />}
				code={code}
				className="max-lg:rounded-t-none"
			>
				{file === "next" ? (
					<>
